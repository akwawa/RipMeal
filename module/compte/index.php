...
if (!empty($_SESSION[$application]['idRank'])) {
	if (!file_exists(dirname(__FILE__).'/private/'.$menu.'.class.php')) {
		...
		if (!empty($lister_user['result'])) {
			...
			$page['body']['contenu'] .= '<table id="table_compte"><thead><tr><th>Id</th><th>Login</th><th>Rang</th><th>Action</th></tr></thead><tfoot><tr><th>Nom</th><th>Rang</th><th colspan="2">Action</th></tr></tfoot><tbody>';
			...
			$page['header']['js']['direct'][] = '$(document).ready(function(){$("#table_compte").DataTable({"sDom":"CRT<\"clear\">lfrtip","bStateSave":true,"oTableTools":{"sSwfPath":"swf/copy_csv_xls_pdf.swf"},"oLanguage":{"sUrl":"js/dataTable.french.txt.js"}});$("#table_compte").dataTable().columnFilter();$(".popup").click(function(){return false;});});';
		}
	}
}
