...
function modifier_compte_valider(form){
	...
	if(result['result'][0]==true) {
		var ligne=$('#ligne_'+id);
		$('#ligne_'+id).remove();
		$('#table_compte > tbody:first').append('<tr id="ligne_'+id+'" role="row"><td>'+id+'</td><td>'+login+'</td><td>'+rank+'</td><td><a class="popup" href="#" onclick="action(this);" data-id="'+id+'">Action</a></td></tr>');
		fermer_details();
		noty_modifSuccess();
	}else{
		noty_modifError();
