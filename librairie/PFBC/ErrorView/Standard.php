...
class Standard extends \PFBC\ErrorView {
	public function applyAjaxErrorResponse() {
		...
		if(errorSize == 1)
			var errorFormat = "L'erreur suivante à été trouvée";
		else
			var errorFormat = "Les "errorSize + " erreurs suivantes ont été trouvées";

		var errorHTML = '<div class="alert alert-error"><a class="close" data-dismiss="alert" href="#">×</a><strong class="alert-heading">' + errorFormat + ':</strong><ul>';
...
JS;
    ...
    public function render() {
        ...
        if(!empty($errors)) {
            ...
            if($size == 1)
                $format = "L'erreur suivante à été trouvée";
            else
                $format = "Les ".$size." erreurs suivantes ont été trouvées";

			echo <<<HTML
			<div class="alert alert-error">
				<a class="close" data-dismiss="alert" href="#">×</a>
				<strong class="alert-heading">$format:</strong>
				<ul><li>$errors</li></ul>
			</div>
HTML;
        }
